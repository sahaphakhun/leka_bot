...
export class KPIService {
   ...
   */
  private async resolveInternalGroupIdOrDefault(
    inputGroupId: string,
  ): Promise<string | null> {
    try {
  ...
  public async getReportSummary(
    ...
    try {
      ...
      const internalGroupId =
        await this.resolveInternalGroupIdOrDefault(groupId);
      if (!internalGroupId) {
        return {
      ...
      for (const t of tasks as any[]) {
        const hist = (t.workflow?.history || []) as Array<{
          action: string;
          at?: Date;
        }>;
  ...
  public async recordTaskCompletion(
    ...
    try {
      ...
      const weekOf = moment(eventDate)
        .tz(config.app.defaultTimezone)
        .startOf("week")
        .toDate();
      const monthOf = moment(eventDate)
        .tz(config.app.defaultTimezone)
        .startOf("month")
        .toDate();
      ...
      console.log(
        `✅ Recorded KPI for task completion: ${task.title} (${completionType})`,
      );
   ...
   */
  public calculateCompletionType(
    task: Task | TaskEntity,
  ): "early" | "ontime" | "late" {
    const dueTime = moment(task.dueTime).tz(config.app.defaultTimezone);
    const completedTime = moment(task.completedAt).tz(
  ...
  private async maybeAwardStreakBonus(params: {
    ...
    try {
      ...
      console.log(
        `🏅 Awarded streak bonus to user ${userId} (streak ${streak})`,
      );
    } catch (error) {
  ...
  public async getGroupLeaderboard(
    ...
    try {
      throttledLogger.log(
        "info",
        `🔍 Getting leaderboard for group: ${groupId}, period: ${period}`,
        "get_leaderboard",
      );
      ...
      let kpiQuery = this.kpiRepository
        ...
        .addSelect(
          "SUM(CASE WHEN kpi.type IN ('assignee_early','assignee_ontime','assignee_late') THEN 1 ELSE 0 END)",
          "assignmentsCompleted",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.type IN ('assignee_early','assignee_ontime') THEN 1 ELSE 0 END)",
          "assignmentsOnTime",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.type = 'assignee_early' THEN 1 ELSE 0 END)",
          "assignmentsEarly",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.type = 'assignee_late' THEN 1 ELSE 0 END)",
          "assignmentsLate",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.type = 'penalty_overdue' THEN 1 ELSE 0 END)",
          "penaltyCount",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.type = 'streak_bonus' THEN kpi.points ELSE 0 END)",
          "bonusPoints",
        )
        .addSelect(
          "SUM(CASE WHEN kpi.role = 'penalty' THEN kpi.points ELSE 0 END)",
          "penaltyPoints",
        )
      ...
      kpiResults.forEach((row: any) => {
        ...
        kpiMap.set(userId, {
          assignmentsCompleted: parseInt(
            row.assignmentsCompleted || row.assignmentscompleted || "0",
          ),
          assignmentsOnTime: parseInt(
            row.assignmentsOnTime || row.assignmentsontime || "0",
          ),
          assignmentsEarly: parseInt(
            row.assignmentsEarly || row.assignmentsearly || "0",
          ),
          assignmentsLate: parseInt(
            row.assignmentsLate || row.assignmentslate || "0",
          ),
          penaltyCount: parseInt(row.penaltyCount || row.penaltycount || "0"),
          bonusPoints: parseFloat(row.bonusPoints || row.bonuspoints) || 0,
          penaltyPoints:
            parseFloat(row.penaltyPoints || row.penaltypoints) || 0,
          totalRawPoints:
            parseFloat(row.totalRawPoints || row.totalrawpoints) || 0,
        });
      });

      const memberMap = new Map(members.map((member) => [member.id, member]));
      const missingIds = Array.from(kpiMap.keys()).filter(
        (id) => !memberMap.has(id),
      );

      let extraUsers: Array<{
        id: string;
        displayName: string;
        lineUserId: string;
      }> = [];
      ...
      if (period === "weekly" && periodStart) {
        creatorQuery = creatorQuery
          .addSelect(
            "SUM(CASE WHEN task.completedAt IS NOT NULL AND task.completedAt >= :periodStart THEN 1 ELSE 0 END)",
            "completedCountPeriod",
          )
          .addSelect(
            "SUM(CASE WHEN task.createdAt >= :periodStart THEN 1 ELSE 0 END)",
            "createdCountPeriod",
          )
      ...
      creatorRows.forEach((row: any) => {
        ...
        if (period === "weekly" || period === "monthly") {
          const createdInPeriod = parseInt(
            row.createdCountPeriod || row.createdcountperiod || "0",
          );
          const completedInPeriod = parseInt(
            row.completedCountPeriod || row.completedcountperiod || "0",
          );
      ...
      for (const member of displayUsers) {
        ...
        try {
          ...
          console.warn(
            `⚠️ Error calculating trend for user ${member.id}:`,
            error,
          );
        }
    ...
    try {
      ...
      for (const member of members) {
        ...
        for (let i = 0; i < recordCount; i++) {
          ...
          const randomEntry =
            completionPool[Math.floor(Math.random() * completionPool.length)];
  ...
  public async getUserWeeklyScoreHistory(
    ...
    Array<{ week: string; averageScore: number; totalTasks: number }>
  > {
    try {
      const history: Array<{
        week: string;
      ...
      for (let i = 0; i < weeks; i++) {
        ...
        const weekEnd = moment(currentWeek)
          .subtract(i - 1, "weeks")
          .toDate();
        ...
        if (result) {
          const averageScore =
            result.averageScore !== null && result.averageScore !== undefined
              ...
              : 0;
          const totalTasks =
            result.totalTasks !== null && result.totalTasks !== undefined
   ...
   * ดึงสถิติตามช่วงเวลา
   */
  public async getStatsByPeriod(
    groupId: string,
    period: "this_week" | "last_week" | "all" = "this_week",
  ): Promise<{
    totalTasks: number;
    ...
    try {
      ...
      if (period !== "all") {
        totalTasksQuery = totalTasksQuery.andWhere(
          "(task.dueTime BETWEEN :startDate AND :endDate) OR (task.createdAt BETWEEN :startDate AND :endDate)",
          { startDate, endDate },
        );
      ...
      const topPerformer =
        leaderboard.length > 0 ? leaderboard[0].displayName : "ไม่มีข้อมูล";
      ...
      if (period !== "all") {
        ...
        if (completedTasksWithTime.length > 0) {
          const totalTime = completedTasksWithTime.reduce(
            (sum: number, task: any) => {
              const diff = moment(task.completedAt)
                .tz(config.app.defaultTimezone)
                .diff(
                  moment(task.dueTime).tz(config.app.defaultTimezone),
                  "hours",
                );
  ...
  public async getUserStats(
    ...
    try {
      ...
      const avgPointsPerTask =
        tasksCompleted > 0 ? totalPoints / tasksCompleted : 0;
  ...
  public async exportKPIData(
    ...
    try {
      ...
      return records.map((record: any) => ({
        วันที่: moment(record.eventDate)
          .tz(config.app.defaultTimezone)
          .format("DD/MM/YYYY"),
        ...
        ประเภท: record.type,
        คะแนน: record.points,
        สัปดาห์: moment(record.weekOf)
          .tz(config.app.defaultTimezone)
          .format("DD/MM/YYYY"),
        เดือน: moment(record.monthOf)
          .tz(config.app.defaultTimezone)
          .format("MM/YYYY"),
      }));
    } catch (error) {
  ...
  private async calculateTrend(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ Timezone error in calculateTrend, using local time:",
          timezoneError,
        );
  ...
  public async getDailyStats(groupId: string): Promise<{
    ...
    try {
      ...
      const pendingTasks =
        totalTasks - completedTasks - overdueTasks - pendingReviewTasks;
      ...
      console.log(
        `   - Sum check: ${completedTasks + overdueTasks + pendingReviewTasks + pendingTasks} = ${totalTasks}`,
      );
  ...
  private async updateMonthlyStats(groupId: string): Promise<void> {
    try {
      ...
      // บันทึกลงฐานข้อมูล (ในอนาคต)
      console.log(
        `📊 Updated monthly stats for group ${groupId}:`,
        monthlyStats,
      );
    } catch (error) {
   ...
   */
  public async updateGroupLeaderboard(
    groupId: string,
    period: "weekly" | "monthly",
  ): Promise<void> {
    try {
      ...
      // บันทึกลงฐานข้อมูล (ในอนาคต)
      console.log(
        `🏆 Updated ${period} leaderboard for group ${groupId}:`,
        leaderboard.length,
        "users",
      );
    } catch (error) {
  ...
  public async syncLeaderboardScores(
    ...
    try {
      throttledLogger.log(
        "info",
        `🔄 Starting leaderboard sync for group: ${groupId}, period: ${period}`,
      );
      // รองรับ LINE Group ID และ 'default' → internal UUID
      const internalGroupId =
        await this.resolveInternalGroupIdOrDefault(groupId);
      ...
      if (period === "weekly" || period === "monthly") {
        ...
        console.log(
          `🔍 Fetching ALL tasks for group ${internalGroupId} (no date filter)`,
        );
      }

      const tasks = await tasksQuery.getMany();
      console.log(
        `📋 Found ${tasks.length} tasks to process for ${period} period`,
      );
      ...
      for (const task of tasks) {
        try {
          ...
          if (task.status === "completed" && task.completedAt) {
            ...
            throttledLogger.log(
              "info",
              `⏰ Processing overdue task: ${task.title} (due: ${moment(task.dueTime).format("DD/MM/YYYY HH:mm")})`,
            ...
            if (task.dueTime && moment(task.dueTime).isAfter(now)) {
              throttledLogger.log(
                "info",
                `⏳ Skipping pending task: ${task.title} (due: ${moment(task.dueTime).format("DD/MM/YYYY HH:mm")})`,
                "skip_pending_task",
              );
            }
          }
      ...
      throttledLogger.forceLog(
        "info",
        `   - Processed tasks: ${processedTasks}`,
      );
      throttledLogger.forceLog(
        "info",
        `   - Updated users: ${processedUsers.size}`,
      );
      throttledLogger.forceLog(
        "info",
      ...
      throttledLogger.forceLog(
        "info",
        `   - On-time completions: ${onTimeCompletions}`,
      );
      throttledLogger.forceLog(
        "info",
        `   - Late completions: ${lateCompletions}`,
      );
      throttledLogger.forceLog(
        "info",
        `   - Penalty records: ${penaltyRecords}`,
      );
  ...
  public async getDebugKPIData(
    ...
    try {
      console.log(
        `🔍 Getting debug KPI data for group: ${groupId}, period: ${period}`,
      );
      ...
      switch (period) {
        case "weekly":
          ...
          const weekEnd = moment()
            ...
            .endOf("week")
            .toDate();
          kpiQuery = kpiQuery.andWhere(
            "kpi.eventDate BETWEEN :weekStart AND :weekEnd",
            { weekStart, weekEnd },
          );
          break;
        case "monthly":
          ...
          const monthEnd = moment()
            ...
            .toDate();
          kpiQuery = kpiQuery.andWhere(
            "kpi.eventDate BETWEEN :monthStart AND :monthEnd",
            { monthStart, monthEnd },
          );
          break;
        // 'all' ไม่ต้องกรอง
