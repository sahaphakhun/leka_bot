...
(function () {
  ...
  document.getElementById("emailStatus").innerHTML = email
    ? `อีเมล: ${email} ✅`
    : "อีเมล: ยังไม่ได้ลิงก์ ❌";
      ...
      fetch(postUrl, {
        ...
        .then((data) => {
          if (data && data.success) {
            ...
            document.getElementById("emailStatus").innerHTML = payload.email
              ? `อีเมล: ${payload.email} ✅`
              : "อีเมล: ยังไม่ได้ลิงก์ ❌";
          } else {
            errorEl.textContent =
              "เกิดข้อผิดพลาด: " +
              (data && data.error ? data.error : "ไม่ทราบสาเหตุ");
...
class Dashboard {
  ...
  bindEvents() {
    ...
    document.querySelectorAll("[data-view-mode]")?.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        const mode =
          target && target.dataset ? target.dataset.viewMode : undefined;
    ...
    document
      .getElementById("submitTaskModalClose")
      ?.addEventListener("click", () => {
        this.closeModal("submitTaskModal");
      });
    document
      .getElementById("cancelSubmitTask")
      ?.addEventListener("click", () => {
        this.closeModal("submitTaskModal");
      });
    document
      .getElementById("submitTaskForm")
    ...
    document
      .getElementById("reviewTaskModalClose")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("fileViewerClose")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("cancelReviewTask")
      ?.addEventListener("click", () => {
        this.closeModal("reviewTaskModal");
      });
    ...
    document
      .getElementById("addTaskModalClose")
      .addEventListener("click", () => {
    ...
    document
      .getElementById("stayHereBtn")
      ?.addEventListener("click", () => this.closeModal("successModal"));

    // Edit task modal handlers
    document
      .getElementById("editTaskModalClose")
      ?.addEventListener("click", () => {
        this.closeModal("editTaskModal");
      });
    document
      .getElementById("editTaskCancelBtn")
      ?.addEventListener("click", () => {
        this.closeModal("editTaskModal");
      });
    ...
    document.getElementById("addTaskForm").addEventListener("submit", (e) => {
      ...
      const submitBtn = e.target.querySelector(
        'button[type="submit"], #addTaskSubmitBtn',
      );
      if (submitBtn && submitBtn.disabled) {
        console.log(
          "⚠️ Form submission already in progress, ignoring duplicate request",
        );
        return;
      }
    ...
    document.getElementById("searchFiles")?.addEventListener(
      "input",
      this.debounce(() => {
        this.filterFiles();
      }, 300),
    ...
    document.querySelectorAll("[data-period]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        const target = e.currentTarget || e.target;
        const period =
          target && target.dataset ? target.dataset.period : undefined;
    ...
    document.querySelectorAll("[data-stats-period]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        const target = e.currentTarget || e.target;
        const period =
          target && target.dataset ? target.dataset.statsPeriod : undefined;
  ...
  async loadStats(period = "this_week") {
    try {
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/stats?period=${period}`,
      );
  ...
  async loadTasks(filters = {}) {
    try {
      ...
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/tasks?${queryParams}`,
      );
      ...
      if (container) {
        container.innerHTML =
          '<div style="text-align: center; padding: 2rem; color: #6b7280;">ไม่สามารถโหลดข้อมูลงานได้</div>';
      }
    }
  }
  ...
  async loadCalendarEvents(month, year) {
    try {
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/calendar?month=${month}&year=${year}`,
      );
  ...
  async loadFiles(search = "") {
    try {
      ...
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/files${queryParams}`,
      );
  ...
  async openUploadPicker() {
    try {
      ...
      input.addEventListener("change", async () => {
        ...
        formData.append(
          "userId",
          this.currentUserId || this.currentUser?.lineUserId || "unknown",
        );
        ...
        try {
          ...
          this.showUploadOverlay({
            subtitle: `${fileArr.length} ไฟล์ • รวม ${this.formatFileSize(totalSize)}`,
          });
          ...
          const data = await this.uploadWithProgress(
            ...
            (loaded, total, lengthComputable) => {
              this.updateUploadOverlay({
                ...
                total,
                lengthComputable,
                detail: `${this.formatFileSize(loaded)} / ${lengthComputable ? this.formatFileSize(total) : "ไม่ทราบขนาด"}`,
              });
            },
          );
  ...
  async loadLeaderboard(period = "weekly") {
    try {
      ...
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/leaderboard?period=${period}`,
      );
      ...
      if (container) {
        container.innerHTML =
          '<div style="text-align: center; padding: 2rem; color: #6b7280;">ไม่สามารถโหลดข้อมูลอันดับได้</div>';
      }
    }
  }
    ...
    try {
      ...
      try {
        ...
        if (lineResponse && lineResponse.data && lineResponse.data.length > 0) {
          console.log(
            `✅ ดึงข้อมูลจาก LINE API สำเร็จ: ${lineResponse.data.length} คน`,
          );
          ...
          if (sourceInfo) {
            const sourceCount = formattedMembers.filter(
              (m) => m.source === "line_api",
            ).length;
            const dbCount = formattedMembers.filter(
              (m) => m.source === "database",
            ).length;
            const webhookCount = formattedMembers.filter(
              (m) => m.source === "webhook",
            ).length;
        ...
        console.warn(
          "⚠️ LINE API ไม่ทำงาน เปลี่ยนไปใช้ฐานข้อมูลแทน:",
          lineError,
        );
        ...
        if (sourceInfo) {
          sourceInfo.textContent =
            "⚠️ LINE API ไม่ทำงาน ใช้ข้อมูลจากฐานข้อมูลแทน";
          sourceInfo.style.display = "block";
        }
      }
  ...
  async uploadInitialFiles(uploaderId) {
    ...
    const formData = new FormData();
    let userId =
      uploaderId || this.currentUserId || this.currentUser?.lineUserId;
    if (!userId) {
    ...
    try {
      ...
      this.showUploadOverlay({
        subtitle: `${files.length} ไฟล์ • รวม ${this.formatFileSize(totalSize)}`,
      });
  ...
  async createTask(taskData) {
    ...
    try {
      ...
      if (this.selectedInitialFiles && this.selectedInitialFiles.length > 0) {
        try {
          const fallbackUploader =
            taskData.createdBy && taskData.createdBy !== "unknown"
              ? taskData.createdBy
      ...
      Object.keys(cleanTaskData).forEach((key) => {
        if (
          cleanTaskData[key] === undefined ||
          cleanTaskData[key] === null ||
          cleanTaskData[key] === ""
        ) {
      ...
      if (
        !cleanTaskData.title ||
        !cleanTaskData.dueTime ||
        !cleanTaskData.assigneeIds ||
        !cleanTaskData.createdBy
      ) {
        ...
        throw new Error(
          "Missing required fields: title, dueTime, assigneeIds, or createdBy",
        );
      }
      ...
      const response = await this.apiRequest(
        `/groups/${this.currentGroupId}/tasks`,
        {
          method: "POST",
      ...
      document.getElementById("successMessage").textContent =
        "บอทจะแจ้งในกลุ่ม LINE เพื่อยืนยันการสร้างงาน";
      this.openModal("successModal");
      this.refreshCurrentView();
      // อัปเดต cache
      try {
        this._taskCache = Array.from(
          new Map(
            [...(this._taskCache || []), response.data].map((t) => [t.id, t]),
          ).values(),
        );
      } catch {}
  ...
  async loadInitialData() {
    try {
      ...
      // Load group info
      const groupResponse = await this.apiRequest(
      ...
      if (
        ...
        this.showToast(
          "ไม่สามารถตรวจสอบสถานะ Bot ได้ กรุณาลองใหม่อีกครั้ง",
          "error",
        );
      } else {
  ...
  async loadUpcomingTasks() {
    try {
      ...
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/tasks?limit=10`,
      );
      ...
      this._taskCache = Array.from(
        new Map(
          [...(this._taskCache || []), ...sortedTasks].map((t) => [t.id, t]),
        ).values(),
      );
  ...
  async loadMiniLeaderboard() {
    try {
      ...
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/leaderboard?period=weekly&limit=3`,
      );
      if (response.data && Array.isArray(response.data)) {
      ...
      if (container) {
        container.innerHTML =
          '<p class="text-muted">ไม่สามารถโหลดข้อมูลอันดับได้</p>';
      }
    }
  }
  ...
  updateMiniLeaderboard(leaderboard) {
    ...
    console.log(
      "🔄 Rendering mini leaderboard with",
      leaderboard.length,
      "users",
    );
    console.log(
    ...
    leaderboard.forEach((user, index) => {
      ...
      // ดึงข้อมูลผู้ใช้
      const name =
        user.displayName || user.name || user.realName || "ไม่ทราบชื่อ";
      const score = Number(
        user.totalScore ||
          user.weeklyPoints ||
          user.monthlyPoints ||
  ...
  updateLeaderboard(users) {
    ...
    if (!users || users.length === 0) {
      container.innerHTML =
        '<div style="text-align: center; padding: 2rem; color: #6b7280;">ยังไม่มีข้อมูลอันดับ</div>';
  ...
  updateTasksList(tasks, pagination) {
    ...
    if (!tasks || tasks.length === 0) {
      container.innerHTML =
        '<div style="text-align: center; padding: 2rem; color: #6b7280;">ไม่พบงานตามเงื่อนไขที่กำหนด</div>';
      return;
    }
    ...
    container.innerHTML = tasks
      .map((task) => {
        const assignees =
          (task.assignedUsers || task.assignees || [])
        ...
        const attachedFilesObjs =
          hasAttachments && typeof task.attachedFiles[0] === "object"
            ? task.attachedFiles
            : [];
  ...
  updateCalendar(events, month, year) {
    ...
    const isCurrentMonth =
      todayAdapter.year() === year && todayAdapter.month() === month - 1;
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dayEvents = events.filter((event) => {
        ...
        if (moment && moment.tz) {
          try {
            eventDate = moment(event.end || event.dueTime || event.start).tz(
              this.timezone,
            );
            eventDateAdapter = {
              year: () => eventDate.year(),
  ...
  updateFilesList(files) {
    ...
    if (!files || files.length === 0) {
      container.innerHTML =
        '<div style="text-align: center; padding: 2rem; color: #6b7280;">ไม่พบไฟล์</div>';
      return;
    }
    ...
    container.innerHTML = files
      .map((file) => {
        ...
        const thumbnailUrl = isImage
          ? `${this.apiBase}/api/groups/${this.currentGroupId}/files/${file.id}/download`
          : null;
  ...
  updateMembersList(members) {
    ...
    if (reviewerSelect) {
      ...
      if (this.currentUserId) {
        const hasCurrent = Array.from(reviewerSelect.options).some(
          (opt) => opt.value === this.currentUserId,
        );
        if (hasCurrent) {
    ...
    if (reportUserSelect) {
      ...
      reportUserSelect.innerHTML =
        '<option value="">ทุกคนในกลุ่ม</option>' +
        members
          .map(
            (member) =>
              `<option value="${member.lineUserId || member.id}">${member.displayName}</option>`,
      ...
      if (
        currentValue &&
        Array.from(reportUserSelect.options).some(
  ...
  getCreatorDisplayName(task) {
    ...
    if (this.groupMembers && this.groupMembers.length > 0) {
      const creator = this.groupMembers.find(
        (m) =>
  ...
  getReviewerDisplayHTML(task) {
    ...
    const reviewerUserId =
      task.reviewerUserId || task.workflow?.review?.reviewerUserId;
    ...
    if (reviewerUserId && this.groupMembers && this.groupMembers.length > 0) {
      const reviewer = this.groupMembers.find(
        (m) =>
  ...
  openAddTaskModal() {
    ...
    if (assignees && assigneeCount && !assignees._boundCount) {
      const update = () => {
        const count = assignees.querySelectorAll(
      ...
      assignees.addEventListener("change", (e) => {
        if (
          e.target &&
          e.target.classList &&
          e.target.classList.contains("assignee-checkbox")
        ) {
          update();
        }
      });
    ...
    if (tagsInput && chips && !tagsInput._boundChips) {
      const render = () => {
        ...
        chips.innerHTML = values
          .map(
            (t, i) =>
              `<span class="chip">#${t}<button type="button" class="remove" data-index="${i}" aria-label="ลบแท็ก ${t}">×</button></span>`,
          )
          .join("");
      };
  ...
  async openTaskModal(taskId) {
    try {
      const task =
        this._taskCache?.find((t) => t.id === taskId) ||
      ...
      try {
        const filesResponse = await this.apiRequest(
          `/api/groups/${this.currentGroupId}/tasks/${taskId}/files`,
        );
          ...
          <div class="task-details">
            ...
            ${
              ...
              <div class="task-attachments">
                ...
                <!-- แยกไฟล์ตามประเภท -->
                ${(() => {
                  const initialFiles = attachedFiles.filter(
                    (f) => f.attachmentType === "initial",
                  );
                  const submissionFiles = attachedFiles.filter(
                    (f) => f.attachmentType === "submission",
                  );
    ...
    try {
      ...
      if (response && response.data && response.data.length > 0) {
        ...
        if (assigneesContainer) {
          assigneesContainer.innerHTML = response.data
            .map(
              (member) => `
          ...
          if (assigneeCount && !assigneesContainer._boundEditCount) {
            const updateCount = () => {
              const count = assigneesContainer.querySelectorAll(
                ".assignee-checkbox:checked",
              ).length;
              assigneeCount.textContent = count > 0 ? `(${count} คน)` : "";
            };
  ...
  async loadTaskForEdit(taskId) {
    try {
      const response = await this.apiRequest(
        `/api/groups/${this.currentGroupId}/tasks/${taskId}`,
      );
      if (response.success && response.data) {
        ...
        document.getElementById("editTaskDueDate").value =
          this.formatDateForForm(task.dueTime);
        document.getElementById("editTaskDueTime").value =
          this.formatTimeForForm(task.dueTime);
        ...
        // เลือกผู้รับผิดชอบ
        const assigneeCheckboxes = document.querySelectorAll(
          "#editTaskAssignees .assignee-checkbox",
        );
        assigneeCheckboxes.forEach((checkbox) => {
          checkbox.checked = task.assignedUsers?.some(
            (user) => user.id === checkbox.value,
          );
        });
        ...
        if (assigneeCount) {
          const count = document.querySelectorAll(
            "#editTaskAssignees .assignee-checkbox:checked",
          ).length;
          assigneeCount.textContent = count > 0 ? `(${count} คน)` : "";
        }
  ...
  async handleAddTask() {
    ...
    if (this._isHandlingAddTask) {
      console.log(
        "⚠️ Add task handling already in progress, ignoring duplicate request",
      );
      return;
    }
    ...
    const originalText =
      submitBtn?.innerHTML || '<i class="fas fa-plus"></i> เพิ่มงาน';
    if (submitBtn) {
      submitBtn.innerHTML =
        '<i class="fas fa-spinner fa-spin"></i> กำลังเพิ่มงาน...';
    ...
    try {
      ...
      const assigneeIds = Array.from(
        document.querySelectorAll("#taskAssignees .assignee-checkbox:checked"),
      ).map((input) => input.value);

      if (assigneeIds.length === 0) {
        this.showFieldError(
          "taskAssignees",
          "กรุณาเลือกผู้รับผิดชอบอย่างน้อย 1 คน",
        );
      ...
      taskData.requireAttachment =
        document.getElementById("requireAttachment").checked;
      ...
      const reviewerUserId =
        document.getElementById("reviewerSelect")?.value || this.currentUserId;
      ...
      if (recurrenceType !== "none") {
        try {
          ...
          const payload = {
            ...
            dayOfMonth:
              recurrenceType === "monthly" || recurrenceType === "quarterly"
                ...
                : undefined,
            timeOfDay:
              document.getElementById("timeOfDayInput").value || "09:00",
          ...
          this.closeModal("addTaskModal");
          document.getElementById("successMessage").textContent =
            "ตั้งค่างานประจำสำเร็จ บอทจะสร้างงานตามรอบเวลาในกลุ่ม LINE";
          this.openModal("successModal");
        } catch (err) {
  ...
  async handleSubmitTask() {
    try {
      ...
      this.showUploadOverlay({
        subtitle: `${selectedFiles.length} ไฟล์ • รวม ${this.formatFileSize(totalSize)}`,
      });
  ...
  async populateSubmitTaskSelect(selectedTaskId = "") {
    try {
      ...
      if (incompleteTasks.length === 0) {
        ...
        sel.innerHTML = incompleteTasks
          .map(
            (t) =>
              `<option value="${t.id}" ${selectedTaskId === t.id ? "selected" : ""}>${t.title} (${this.getStatusText(t.status)})</option>`,
          )
          .join("");
      }
    } catch (error) {
  ...
  async populateReviewTaskSelect(selectedTaskId = "") {
    try {
      ...
      const response = await this.apiRequest(
        `/groups/${this.currentGroupId}/tasks?status=pending`,
      );
      const response2 = await this.apiRequest(
        `/groups/${this.currentGroupId}/tasks?status=in_progress`,
      );
      ...
      this._taskCache = Array.from(
        new Map(
          [...(this._taskCache || []), ...tasks].map((t) => [t.id, t]),
        ).values(),
      );
      ...
      if (tasks.length === 0) {
        ...
        sel.innerHTML = tasks
          .map(
            (t) =>
              `<option value="${t.id}" ${selectedTaskId === t.id ? "selected" : ""}>${t.title}</option>`,
          )
          .join("");
      }
    } catch (error) {
    ...
    try {
      ...
      if (this.isMomentAvailable()) {
        try {
          ...
          currentYear =
            parseInt(parts[1]) ||
            this.adjustToThailandTimezone(new Date()).getFullYear();
        }
      } else {
        currentYear =
          parseInt(parts[1]) ||
  ...
  onCalendarDayClick(year, month, day) {
    ...
    this.apiRequest(
      `/groups/${this.currentGroupId}/calendar?month=${month}&year=${year}`,
    )
      .then((resp) => {
        const events = (resp.data || []).filter((ev) => {
          ...
          if (moment && moment.tz) {
            try {
              eventDate = moment(ev.end || ev.dueTime || ev.start).tz(
                this.timezone,
              );
              eventDateAdapter = {
                year: () => eventDate.year(),
        ...
        if (!body) return;
        if (events.length === 0) {
          this.showToast("ไม่มีงานในวันนี้", "info");
          return;
        }
  ...
  async downloadFile(fileId, retryAttempt = 0) {
    ...
    try {
      ...
      if (retryAttempt > 0) {
        this.showToast(
          `กำลังลองดาวน์โหลดอีกครั้ง... (ครั้งที่ ${retryAttempt + 1})`,
          "info",
        );
      }

      const response = await fetch(
        `${this.apiBase}/api/groups/${this.currentGroupId}/files/${fileId}/download`,
      );
      ...
      if (response.ok) {
        ...
        if (filename === "download") {
          try {
            ...
            const infoRes = await fetch(
              `${this.apiBase}/api/groups/${this.currentGroupId}/files/${fileId}`,
            );
            if (infoRes.ok) {
          ...
          if (filename === "download") {
            const nameFromUI = document
              .querySelector(`[data-file-id="${fileId}"] .file-name`)
              ?.textContent?.trim();
        ...
        if (shouldRetry && retryAttempt < maxRetries) {
          console.warn(
            `Download failed with ${response.status}, retrying... (attempt ${retryAttempt + 1}/${maxRetries})`,
          );
          await new Promise((resolve) =>
            setTimeout(resolve, retryDelay * (retryAttempt + 1)),
          );
      ...
      if ((isNetworkError || isServerError) && retryAttempt < maxRetries) {
        console.warn(
          `Network error, retrying... (attempt ${retryAttempt + 1}/${maxRetries})`,
        );
        await new Promise((resolve) =>
          setTimeout(resolve, retryDelay * (retryAttempt + 1)),
        );
  ...
  filterFiles() {
    ...
    const searchTerm =
      document.getElementById("searchFiles")?.value.toLowerCase() || "";
    ...
    let filtered = this.allFiles.filter((file) => {
      ...
      const matchesTask =
        ...
        (taskFilter !== "no-task" &&
          file.taskIds &&
          file.taskIds.includes(taskFilter));

      // กรองตามประเภทไฟล์
      const matchesType =
        !typeFilter || this.getFileCategory(file.mimeType) === typeFilter;
  ...
  getFileCategory(mimeType) {
    ...
    // Database
    if (mimeType.includes("sqlite") || mimeType.includes("access"))
      return "database";
  ...
  populateTaskFilter(tasks) {
    ...
    const taskOptions = tasksWithFiles
  ...
  getFileIcon(mimeType) {
    ...
    if (mimeType.includes("illustrator") || mimeType.includes("postscript"))
      return "fa-file-image";
    if (
      mimeType.includes("indesign") ||
      mimeType.includes("figma") ||
  ...
  async loadDashboardData() {
    try {
      ...
      if (tasksResponse.status === "fulfilled" && tasksResponse.value?.data) {
        const taskPayload = tasksResponse.value.data;
        const recentTasks = Array.isArray(taskPayload?.data)
          ? taskPayload.data
      ...
      if (statsResponse.status === "fulfilled" && statsResponse.value?.data) {
        const statsPayload =
          statsResponse.value.data?.stats || statsResponse.value.data;
        this.updateDashboardStats(statsPayload);
      }
      ...
      if (
        ...
        console.log(
          "📊 Leaderboard response fulfilled:",
        ...
        console.warn(
          "⚠️ ไม่สามารถโหลด Leaderboard ได้:",
          leaderboardResponse.reason,
        );
        // แสดงข้อความในหน้า dashboard
        const container = document.getElementById("miniLeaderboard");
        if (container) {
        ...
        console.warn(
          "⚠️ Leaderboard response status:",
          leaderboardResponse.status,
        );
        console.warn(
          "⚠️ Leaderboard response value:",
          leaderboardResponse.value,
        );
      }
  ...
  updateDashboardStats(stats) {
    try {
      if (!stats || typeof stats !== "object") {
        console.warn("⚠️ No stats data provided for dashboard update:", stats);
        return;
      }

      const resolvedStats =
        stats.stats && typeof stats.stats === "object" ? stats.stats : stats;
      console.log("🔄 Updating dashboard stats:", resolvedStats);
      ...
      if (totalTasksEl)
        totalTasksEl.textContent = toNumber(resolvedStats.totalTasks);
      if (completedTasksEl)
        completedTasksEl.textContent = toNumber(resolvedStats.completedTasks);
      if (pendingTasksEl)
        pendingTasksEl.textContent = toNumber(resolvedStats.pendingTasks);
      if (overdueTasksEl)
        overdueTasksEl.textContent = toNumber(resolvedStats.overdueTasks);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("openSubmitFromDashboardBtn")
    ?.addEventListener("click", () => {
