...
async function initializeDatabase() {
    try {
        ...
        console.log('📊 [INIT] Environment:', config_1.config.nodeEnv);
        // Connect to database
    ...
    `);
        console.log('📋 [INIT] Existing tables:', existingTables.map((t) => t.table_name));
      ...
      WHERE table_schema = 'public'
    `);
        console.log('📋 [INIT] Final tables:', finalTables.map((t) => t.table_name));
        // Expected tables
        const expectedTables = ['users', 'groups', 'tasks', 'files', 'kpi_records', 'task_assignees'];
        const missingTables = expectedTables.filter(table => !finalTables.some((t) => t.table_name === table));
