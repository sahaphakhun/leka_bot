...
class ComprehensiveMigration {
  ...
  async runComprehensiveMigration() {
    ...
    try {
      ...
      if (!database_1.AppDataSource.isInitialized) {
        logger_1.logger.info(
          "⏳ Database not initialized yet, initializing...",
        );
        try {
      ...
      for (const step of migrationSteps) {
        try {
          ...
          logger_1.logger.warn(
            `⚠️ Continuing with remaining migration steps...`,
          );
        }
      }
      // Report results
      ...
      if (failureCount > 0) {
        logger_1.logger.warn(
          "⚠️ Some migration steps failed. System may have reduced functionality.",
        );
  ...
  async validateDatabaseSchema() {
    ...
    try {
      ...
      logger_1.logger.info(
        `✅ Database schema validation passed. Found ${tableNames.length} tables.`,
      );
    } finally {
  ...
  async addMissingColumns() {
    ...
    try {
      ...
      await this.addMissingColumnsToTable(
        queryRunner,
        "tasks",
        taskColumns,
        requiredTaskColumns,
      ...
      const recurringTaskColumns = await this.getTableColumns(
        queryRunner,
        "recurring_tasks",
      );
   ...
   */
  async addMissingColumnsToTable(
    queryRunner,
    tableName,
    existingColumns,
    requiredColumns,
    ...
    for (const column of requiredColumns) {
      if (!existingColumns.includes(column.name)) {
        try {
          ...
          logger_1.logger.warn(
            `⚠️ Failed to add column ${tableName}.${column.name}:`,
            error,
          );
        }
      }
    ...
    if (addedCount === 0) {
      ...
      logger_1.logger.info(
        `✅ Added ${addedCount} missing columns to ${tableName} table`,
      );
    }
  }
  /**
  ...
  async fixEnumValues() {
    ...
    try {
      ...
      for (const status of missingStatuses) {
        try {
          await queryRunner.query(
            `ALTER TYPE tasks_status_enum ADD VALUE '${status}'`,
          );
          logger_1.logger.info(`✅ Added task status enum value: ${status}`);
        } catch (error) {
          logger_1.logger.warn(
            `⚠️ Failed to add task status enum value ${status}:`,
            error,
          );
        }
      }
      ...
      if (kpiEnumValues.length > 0) {
        ...
        for (const kpiType of requiredKpiTypes) {
          if (!existingKpiTypes.includes(kpiType)) {
            try {
              await queryRunner.query(
                `ALTER TYPE kpi_records_type_enum ADD VALUE '${kpiType}'`,
              );
              logger_1.logger.info(`✅ Added KPI enum value: ${kpiType}`);
            } catch (error) {
              logger_1.logger.warn(
                `⚠️ Failed to add KPI enum value ${kpiType}:`,
                error,
              );
            }
          }
        ...
        logger_1.logger.info(
          "ℹ️ KPI record type column is not an enum, skipping enum value adjustments",
        );
      }
      ...
      for (const mapping of legacyTypeMappings) {
        try {
          ...
          logger_1.logger.warn(
            `⚠️ Failed to remap KPI record type from ${mapping.from} to ${mapping.to}:`,
            error,
          );
        }
      }
  ...
  async migrateTaskWorkflow() {
    const taskRepository = database_1.AppDataSource.getRepository(
      models_1.Task,
    );
    ...
    for (const task of tasks) {
      try {
        ...
        logger_1.logger.warn(
          `⚠️ Failed to update workflow for task ${task.id}:`,
          error,
        );
      }
    }
  ...
  async fixUserData() {
    ...
    for (const user of users) {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        fixedCount++;
        logger_1.logger.info(
          `✅ Fixed user data for ${user.id}: ${JSON.stringify(updates)}`,
        );
      }
    }
  ...
  async fixGroupData() {
    ...
    for (const group of groups) {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        fixedCount++;
        logger_1.logger.info(
          `✅ Fixed group data for ${group.id}: ${group.name}`,
        );
      }
    }
  ...
  async fixTaskAssignments() {
    ...
    try {
      ...
      if (orphanedAssignments.length > 0) {
        logger_1.logger.warn(
          `⚠️ Found ${orphanedAssignments.length} orphaned task assignments`,
        );
        ...
        `);
        logger_1.logger.info(
          `✅ Removed ${orphanedAssignments.length} orphaned task assignments`,
        );
      }
      ...
      for (const task of tasksWithoutAssignees) {
        try {
          ...
          logger_1.logger.info(
            `✅ Assigned task ${task.id} to creator ${task.createdBy}`,
          );
        } catch (error) {
          logger_1.logger.warn(
            `⚠️ Failed to assign task ${task.id} to creator:`,
            error,
          );
        }
      }
  ...
  async fixFileAttachments() {
    const fileRepository = database_1.AppDataSource.getRepository(
      models_1.File,
    );
    ...
    if (result.affected && result.affected > 0) {
      logger_1.logger.info(
        `✅ Set default attachmentType for ${result.affected} files`,
      );
    }
  ...
  async fixKPIRecords() {
    ...
    try {
      ...
      if (orphanedKPIs.length > 0) {
        ...
        `);
        logger_1.logger.info(
          `✅ Removed ${orphanedKPIs.length} orphaned KPI records`,
        );
      }
    ...
    for (const task of tasks) {
      try {
        ...
        if (newStatus !== task.status) {
          ...
          updatedCount++;
          logger_1.logger.info(
            `✅ Updated task ${task.id} status: ${task.status} → ${newStatus}`,
          );
        }
      } catch (error) {
        logger_1.logger.warn(
          `⚠️ Failed to update status for task ${task.id}:`,
          error,
        );
      }
    }
  ...
  async validateDataIntegrity() {
    ...
    try {
      ...
      const taskCount = await queryRunner.query(
        "SELECT COUNT(*) as count FROM tasks",
      );
      const fileCount = await queryRunner.query(
      ...
      if (parseInt(tasksWithoutCreator[0].count) > 0) {
        logger_1.logger.warn(
          `⚠️ Found ${tasksWithoutCreator[0].count} tasks without valid creator`,
        );
      }
  ...
  async checkMigrationNeeded() {
    try {
      ...
      try {
        ...
        if (missingTables.length > 0) {
          logger_1.logger.info(
            `📋 Missing tables detected: ${missingTables.join(", ")} - migration needed`,
          );
          return true;
        }
        ...
        const tasksMissingColumns = requiredTaskColumns.some(
          (col) => !taskColumns.includes(col),
        );
        const fileColumns = await this.getTableColumns(queryRunner, "files");
        const filesMissingAttachmentType =
          !fileColumns.includes("attachmentType");
        const recurringTaskColumns = await this.getTableColumns(
          queryRunner,
          "recurring_tasks",
        );
        const recurringTasksMissingDurationDays =
          !recurringTaskColumns.includes("durationDays");
        const recurringTasksMissingTotalInstances =
          !recurringTaskColumns.includes("totalInstances");
        const kpiColumns = await this.getTableColumns(
          queryRunner,
          "kpi_records",
        );
        ...
        `);
        const hasTasksWithoutWorkflow =
          parseInt(tasksWithoutWorkflow[0].count) > 0;
        ...
        if (migrationNeeded) {
          logger_1.logger.info(
            "📋 Migration needed due to missing columns or data issues",
          );
        } else {
          logger_1.logger.info(
