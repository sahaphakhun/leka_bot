...
class WebhookController {
    constructor() {
        ...
        this.notificationService = serviceContainer_1.serviceContainer.get('NotificationService');
        this.commandService = serviceContainer_1.serviceContainer.get('CommandService');
    }
    /**
    ...
    async processEvent(event) {
        try {
            logger_1.logger.info('Processing event:', { type: event.type, source: event.source });
            switch (event.type) {
                case 'message':
                ...
                case 'memberLeft':
                    ...
                    break;
                default:
                    logger_1.logger.info('Unhandled event type:', { type: event.type });
            }
        }
        catch (error) {
    ...
    async handleMessageEvent(event) {
        ...
        switch (message.type) {
            ...
            case 'audio':
            case 'file':
                await this.handleFileMessage(event, message);
                break;
            default:
     ...
     */
    async handleTextMessage(event, text) {
                ...
                try {
                    ...
                    if (user) {
                        ...
                        if (allTasks.length > 0) {
                            ...
                            const allTasksCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createAllPersonalTasksCard(allTasks, files, user, overdueTasks);
                            const guideText = '💡 **วิธีการส่งงาน (มาตรฐานใหม่):**\n\n' +
                                '✨ **ขั้นตอนการส่งงาน:**\n' +
            ...
            if (trimmedText === 'flow' || trimmedText === 'Flow' || trimmedText === 'FLOW') {
                try {
                    ...
                    if (user) {
                        ...
                        const flowCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createTaskSubmissionFlowCard(user);
        ...
        if (command) {
            ...
            try {
                ...
                await this.safeReplyError(replyToken, 'เกิดข้อผิดพลาดในการประมวลผลคำสั่ง กรุณาลองใหม่อีกครั้ง');
            }
        }
        else {
...
🔧 เริ่มต้น: แท็กบอท /help เพื่อดูคำสั่งทั้งหมด`;
     ...
     */
    async handleFileMessage(event, message) {
        const { source, replyToken } = event;
        // ตรวจสอบประเภทของ source
        if (source.type === 'user') {
            ...
            try {
                ...
                if (savedFile) {
                    ...
                    if (user) {
                        ...
                        const fileListCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createPersonalFileListCard(files, user, undefined);
                        await this.lineService.replyMessage(replyToken, fileListCard);
                        logger_1.logger.info('File saved and file list shown:', {
                            fileId: savedFile.id,
                ...
                logger_1.logger.error('Error saving file in personal chat:', error);
                await this.lineService.replyMessage(replyToken, '❌ ไม่สามารถบันทึกไฟล์ได้ กรุณาลองใหม่อีกครั้ง');
            }
        }
        else if (source.type === 'group') {
            // แชทกลุ่ม - ไม่บันทึกอัตโนมัติ
            logger_1.logger.info('File message received in group chat but ignored (auto-save disabled)', {
            ...
            switch (action) {
                    ...
                    if (taskId) {
                        try {
                            ...
                            if (task.requireAttachment) {
                                ...
                                const fileAttachmentCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createFileAttachmentCard(task, group, assignee);
                        ...
                        if (source.type === 'user' && fileIds.length === 0) {
                            ...
                            if (user) {
                                ...
                                try {
                                    ...
                                    finalFileIds = result.files?.map((f) => f.id) || [];
                                }
                                catch (error) {
                ...
                case 'confirm_submit': {
                    ...
                    try {
                        ...
                        const submitConfirmationCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createSubmitConfirmationCard(task, group, files.length, files.map((f) => f.originalName));
                    ...
                    if (taskId) {
                        try {
                            ...
                            if (user) {
                                ...
                                const confirmationCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createTaskSubmissionConfirmationCard(task, files, user);
                ...
                case 'edit': {
                    ...
                    if (taskId && editGroupId) {
                        try {
                            ...
                            const editUrl = `${config_1.config.baseUrl}/dashboard?groupId=${editGroupId}&taskId=${taskId}&action=edit&userId=${userId}`;
                        ...
                        if (user) {
                            ...
                            if (allTasks.length > 0) {
                                if (allTasks.length === 1) {
                                    ...
                                    const taskWithFilesCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createPersonalTaskWithFilesCard(task, files, user);
                                    await this.lineService.replyMessage(replyToken, taskWithFilesCard);
                                    // แสดงการ์ดรายการไฟล์พร้อม taskId
                                    const fileListCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createPersonalFileListCard(files, user, task.id);
                                    await this.lineService.replyMessage(replyToken, fileListCard);
                                }
                                else {
                ...
                case 'clear_personal_files': {
                    try {
                        ...
                        if (user) {
                            ...
                            const oldFiles = files.filter((f) => new Date(f.uploadedAt) < since);
                    ...
                    try {
                        ...
                        if (user) {
                            ...
                            if (allTasks.length > 5) {
                                ...
                                const moreTasksCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createAllPersonalTasksCard(remainingTasks, files, user, overdueTasks);
                        ...
                        try {
                            if (taskId && groupId) {
                                ...
                                if (files.length === 0) {
                                    ...
                                    files = allGroupFiles.files.filter(file => file.linkedTasks && file.linkedTasks.some((t) => t.id === taskId));
                                }
                                // ถ้ายังไม่พบไฟล์อีก ให้ fallback ไปดูจาก workflow.submissions ล่าสุด (ดึงตาม fileIds)
                                if (files.length === 0 && task && task.workflow && Array.isArray(task.workflow.submissions)) {
                                ...
                                if (task && files.length > 0) {
                                    ...
                                    const taskFilesCard = FlexMessageTemplateService_1.FlexMessageTemplateService.createTaskFilesCategorizedCard(task, filesByType, { id: groupId });
                ...
                case 'link_files': {
                    ...
                    if (fileIdsParam) {
                        ...
                        const taskButtons = tasks.slice(0, 5).map((task) => FlexMessageDesignSystem_1.FlexMessageDesignSystem.createButton(`${task.title} (${task.id.substring(0, 8)})`, 'postback', `action=link_files_to_task&taskId=${task.id}&fileIds=${fileIds.join(',')}`, 'secondary'));
                        const footerButtons = [
                            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createButton('ไม่ผูกกับงาน', 'postback', `action=no_link_files&fileIds=${fileIds.join(',')}`, 'primary')
                        ];
                        const flexMessage = FlexMessageDesignSystem_1.FlexMessageDesignSystem.createStandardTaskCard('📋 เลือกงานเพื่อผูกไฟล์', '📋', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.primary, [
                            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`ไฟล์ ${fileIds.length} รายการ`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textSecondary),
                            ...taskButtons
                        ], footerButtons, 'extraLarge');
                ...
                case 'view_details': {
                    ...
                    if (taskId) {
                        try {
                            ...
                            const taskDetailUrl = `${config_1.config.baseUrl}/dashboard/task/${taskId}`;
                    ...
                    if (taskId && groupId) {
                        try {
                            ...
                            if (creator && creator.lineUserId) {
                                ...
                                await this.lineService.replyMessage(replyToken, `✅ ส่งคำขอเลื่อนเวลาไปยังผู้สร้างงานแล้ว\n\n` +
                                    `📋 งาน: ${task.title}\n` +
                    ...
                    if (taskId && requesterId) {
                        try {
                            ...
                            if (requester && requester.lineUserId) {
                                ...
                                await this.notificationService.sendExtensionRejectedNotification(task, requester);
                            }
     ...
     */
    async safeReplyError(replyToken, errorMessage) {
        try {
            // ตรวจสอบ replyToken
...
พิมพ์ "@เลขา /help" เพื่อดูคำสั่งทั้งหมดค่ะ ✨`;
     ...
     */
    async handlePersonalTaskRequest(event) {
        try {
            const { source, replyToken } = event;
            const userId = source.userId;
            ...
            if (userTasks.length === 0) {
                await this.lineService.replyMessage(event.replyToken, '📋 คุณไม่มีงานที่ต้องทำในขณะนี้ค่ะ\n\n💡 งานจะแสดงที่นี่เมื่อคุณได้รับมอบหมายงานใหม่');
                return;
            }
            // จัดกลุ่มงานตามกลุ่ม
            ...
            await this.lineService.replyMessage(event.replyToken, 'เกิดข้อผิดพลาดในการดึงข้อมูลงาน กรุณาลองใหม่อีกครั้ง');
        }
    }
    /**
     * จัดกลุ่มงานตามกลุ่ม
 ...
 */
    createPersonalTasksFlexMessage(tasksByGroup) {
        const groupEntries = Object.entries(tasksByGroup);
        const content = [
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`📋 งานที่ต้องทำของคุณ (${Object.values(tasksByGroup).reduce((total, group) => total + group.tasks.length, 0)} รายการ)`, 'lg', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textPrimary, 'bold', true),
            // เพิ่มคำอธิบายสถานะงาน
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText('⏳ รอทำ | 🔄 กำลังทำ | ⚠️ เกินกำหนด', 'xs', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textSecondary, undefined, true)
        ];
        // เพิ่มงานแต่ละกลุ่ม
        for (const [groupId, groupData] of groupEntries) {
            // หัวข้อกลุ่ม - ใช้ชื่อกลุ่มจริง
            content.push(FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`🏷️ ${groupData.groupName}`, 'md', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.primary, 'bold', true));
            // งานในกลุ่ม
            for (const task of groupData.tasks.slice(0, 10)) { // แสดง 10 งานแรก
                const statusEmoji = this.getTaskStatusEmoji(task.status);
                const dueDate = task.dueTime ? (0, moment_timezone_1.default)(task.dueTime).format('DD/MM HH:mm') : 'ไม่มีกำหนด';
                // ใช้ "|" ขีดคั่นบอกสถานะงาน
                content.push(FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`${statusEmoji} ${task.title} | ⏰ ${dueDate}`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textPrimary));
            }
            // แสดงจำนวนงานที่เหลือ
            if (groupData.tasks.length > 10) {
                content.push(FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`... และอีก ${groupData.tasks.length - 10} งาน`, 'xs', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textSecondary, undefined, true));
            }
        ...
        // ปุ่มดูงานทั้งหมด
        const buttons = [
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createButton('ดูงานทั้งหมดในเว็บ', 'uri', `${config_1.config.baseUrl}/dashboard/my-tasks`, 'primary')
        ];
        return FlexMessageDesignSystem_1.FlexMessageDesignSystem.createStandardTaskCard('📋 งานที่ต้องทำของคุณ', '📋', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.primary, content, buttons, 'extraLarge');
    }
    /**
     * ได้ emoji สถานะงาน
     */
     ...
     */
    async ensureUserAndGroup(userId, groupId) {
        try {
     ...
     * ตรวจสอบและบันทึกสมาชิกใหม่ที่ส่งข้อความ
     */
    async checkAndSaveNewMemberFromMessage(groupId, userId) {
        try {
     ...
     */
    async createExtensionRequestCard(task, groupId, requesterId) {
        const requester = await this.userService.findByLineUserId(requesterId);
        const dueDate = (0, moment_timezone_1.default)(task.dueTime).tz(config_1.config.app.defaultTimezone).format('DD/MM/YYYY HH:mm');
        const content = [
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`📋 งาน: ${task.title}`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textPrimary, 'bold'),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`📅 กำหนดส่งเดิม: ${dueDate}`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textPrimary),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`👤 ผู้ขอ: ${requester?.displayName || 'ไม่ทราบ'}`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textPrimary),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText(`📝 เหตุผล: ขอเลื่อนเวลาส่งงาน`, 'sm', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textSecondary),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createSeparator('medium'),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createText('💡 กดปุ่มด้านล่างเพื่ออนุมัติและเลือกวันกำหนดงานใหม่', 'xs', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.textSecondary)
        ];
        const buttons = [
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createButton('อนุมัติและเลือกวันใหม่', 'uri', `${config_1.config.baseUrl}/dashboard?groupId=${groupId}&taskId=${task.id}&action=approve_extension`, 'primary'),
            FlexMessageDesignSystem_1.FlexMessageDesignSystem.createButton('ปฏิเสธ', 'postback', `action=reject_extension&taskId=${task.id}&requesterId=${requesterId}`, 'secondary')
        ];
        return FlexMessageDesignSystem_1.FlexMessageDesignSystem.createStandardTaskCard('⏰ คำขอเลื่อนเวลา', '⏰', FlexMessageDesignSystem_1.FlexMessageDesignSystem.colors.warning, content, buttons, 'extraLarge');
    }
}
